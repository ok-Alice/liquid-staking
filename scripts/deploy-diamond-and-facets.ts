namespace deployDiamond {
    ...
    const getSelectorsFromMessages = (messages: typeof AbiMessage[]): number[][] => {
        return messages.map((message) => {
            return message.selector.toU8a() as unknown as number[]
        })
    }

    const getSelectorsFromMessagesString = (messages: typeof AbiMessage[]): string[] => {
        return messages.map((message) => {
            return message.selector.toString()
        })
    }

    const getSelectorByName = (messages: typeof AbiMessage[], name: string): number[] => {
        return messages.filter((message) => {
            return message.identifier == name
        })[0].selector.toU8a() as unknown as number[]
    }

    const getSelectorByNameString = (messages: typeof AbiMessage[], name: string): string => {
        return messages.filter((message) => {
            return message.identifier == name
        })[0].selector.toString()
    }
...
;
            ...
            console.log(await api.query.contracts.contractInfoOf(
                contractAddress,
            ));
    ...
    async function main() {
        ...
        const issuerFacetContract = await deployContract(api, alice, "issuer_facet_v1");

        const issuerFacetHash = issuerFacetContract.abi.info.source.wasmHash.toString();
        console.log("issuer_facet_v1 contract hash:", issuerFacetHash);

        const issuerFacetInit = getSelectorByName(issuerFacetContract.abi.messages, 'init_psp22');
        const issuerFacetSelectors = getSelectorsFromMessages(issuerFacetContract.abi.messages);
        ...
        const stakerFacetContract = await deployContract( api, alice, "staker_facet_v1");

        const stakerFacetHash = stakerFacetContract.abi.info.source.wasmHash.toString();
        console.log("staker_facet_v1 contract hash:", stakerFacetHash);

        const stakerFacetInit = getSelectorByName(stakerFacetContract.abi.messages, 'pause');
        const stakerFacetSelectors = getSelectorsFromMessages(stakerFacetContract.abi.messages);
        ...
        const diamondCallerContract = await deployContract( api, alice, "diamond_caller");

        const diamondCallerHash = diamondCallerContract.abi.info.source.wasmHash.toString();
        console.log("diamond_caller_v1 contract hash:", diamondCallerHash);

        const diamondCallerInit = getSelectorByName(diamondCallerContract.abi.messages, 'balance_of');
        const diamondCallerSelectors = getSelectorsFromMessages(diamondCallerContract.abi.messages);
        ...
        const diamond_contract = await deployContract( api, alice, "delegator", "./target/ink", alice.address);
        console.log(`diamond contract: ${diamond_contract.address}`);

        const diamond_owner = await contractQuery(api, alice.address, diamond_contract, "ownable::owner");
        ...
        // const diamond_cut_result_issuer_facet = await contractTx(alice, diamond_contract, 'diamond::diamondCut',
        //     issuerFacetCut, issuerFacetCutInit );
        // console.log(`diamond_cut add issuer facet:  ${diamond_cut_result_issuer_facet.status}`);

        // const diamond_cut_result_staker_facet = await contractTx(alice, diamond_contract, 'diamond::diamondCut',
        //     stakerFacetCut, stakerFacetCutInit );
        // console.log(`diamond_cut add staker facet:  ${diamond_cut_result_staker_facet.status}`);

        // const diamond_cut_result_diamond_caller_facet = await contractTx(alice, diamond_contract, 'diamond::diamondCut',
        //     diamondCallerCut, diamondCallerCutInit );
        // console.log(`diamond_cut add staker facet:  ${diamond_cut_result_diamond_caller_facet.status}`);

        // const facets = await contractQuery(alice.address, diamond_contract, 'diamondLoupe::facets')
