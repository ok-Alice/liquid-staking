...
// I don't understand why?
    const contractCodeHash = async (
        ...
        return new Promise(async (resolve, reject) => {
            ...
            var result = await api.query.contracts.contractInfoOf(
                contractAddress,
            );
    ...
    async function main() {
        ...
        const issuerFacetInit = getSelectorByName(issuerFacetContract.abi.messages, 'default');
        ...
        const stakerFacetInit = getSelectorByName(stakerFacetContract.abi.messages, 'init_psp22');
        ...
        const diamond_contract = await deployContract( api, alice, "diamond", alice.address);
        ...
        console.log(`diamond_cut add Flipper facet:  ${diamond_cut_result_issuer_facet.status}`);
        ...
        console.log(`diamond_cut add PSP22 facet:  ${diamond_cut_result_staker_facet.status}`);
        ...
        issuerFacetContract.address = diamond_contract.address;

        const balance_alice = await contractQuery(alice.address, issuerFacetContract, "psp22::balanceOf", alice.address);
